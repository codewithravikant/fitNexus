...
| Email | Brevo API (`BREVO_API_KEY`) or SMTP (e.g. Gmail with App Password) |
...
**Email on Railway:** Many platforms **block outbound SMTP** (ports 465/587), so direct Gmail SMTP often returns **`ETIMEDOUT`**. Prefer **[Brevo](https://www.brevo.com/)** transactional email via **`BREVO_API_KEY`** (HTTPS) plus **`EMAIL_FROM`** with a Brevo-verified sender. Alternatives: an **SMTP relay** your provider allows, **self-host** where outbound SMTP is permitted, or run the app **locally/Docker** with Gmail SMTP for development.
...
| `BREVO_API_KEY` | [Brevo](https://app.brevo.com/) → SMTP & API → API Keys. Sends mail over HTTPS (good when SMTP ports are blocked). Takes precedence over SMTP when set. |
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` | Outbound email when `BREVO_API_KEY` is unset. Gmail often uses an [App Password](https://myaccount.google.com/apppasswords). |
| `EMAIL_FROM` | From address, e.g. `FitNexus <you@gmail.com>`. With Brevo, use an address **verified in Brevo**. |
...
| **`/signup`** | Registration—email/password or OAuth. Verification email uses Brevo API or SMTP when configured. |
